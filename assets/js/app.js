...
const firebaseConfig = {
  ...
  databaseURL:
    "https://okmusic-ec226-default-rtdb.europe-west1.firebasedatabase.app",
...
window.toggleDisplayMode = () => {
  ...
  if (document.body.classList.contains("display-mode")) {
    ...
    document.getElementById("playerWrapper").style.transform =
...
class AudioExporter {
  async exportAudio() {
    ...
    try {
      ...
      const blob = await fetch(URL.createObjectURL(track.blob)).then((r) =>
        r.arrayBuffer(),
      );
      ...
      bass.gain.value = window.audioSys.bassVal;
      const reverb = renderCtx.createConvolver();
      const impulse = renderCtx.createBuffer(2, 44100 * 3, 44100);
      for (let i = 0; i < 44100 * 3; i++) {
        impulse.getChannelData(0)[i] =
          (Math.random() * 2 - 1) * Math.exp((-3 * i) / (44100 * 3));
        impulse.getChannelData(1)[i] =
          (Math.random() * 2 - 1) * Math.exp((-3 * i) / (44100 * 3));
      }
      ...
      src.connect(bass);
      bass.connect(renderCtx.destination);
      bass.connect(reverb);
      reverb.connect(revGain);
      revGain.connect(renderCtx.destination);
      src.start();
...
class MusicDB {
  ...
  async markAsSeen(id) {
    return this.update(id, { isNew: false });
  }
}
...
class LibraryManager {
  ...
  save() {
    localStorage.setItem("sv_library_struct", JSON.stringify(this.structure));
  }
  sync(songs) {
    ...
    const clean = (list) => {
      for (let i = list.length - 1; i >= 0; i--) {
        if (list[i].type === "song") {
          if (!dbIds.has(list[i].id)) list.splice(i, 1);
          else structIds.add(list[i].id);
  ...
  openFolderModal() {
    window.$("folderModal").style.opacity = "1";
    window.$("folderModal").style.pointerEvents = "auto";
    window.$("folderNameInput").value = "";
    window.$("folderNameInput").focus();
  }
  closeFolderModal() {
    window.$("folderModal").style.opacity = "0";
    window.$("folderModal").style.pointerEvents = "none";
  }
  confirmCreateFolder() {
    const name = window.$("folderNameInput").value || "New Folder";
    const id = crypto.randomUUID();
  ...
  deleteItem(id) {
    const remove = (list) => {
      ...
      if (idx > -1) {
        ...
        else if (item.type === "folder" && item.items.length > 0)
          this.structure.push(...item.items);
        list.splice(idx, 1);
        return true;
      }
      for (let item of list)
        if (item.type === "folder" && remove(item.items)) return true;
      return false;
    };
  ...
  removeItemFromStructure(id, list = this.structure) {
    ...
    for (const item of list) {
      if (
        item.type === "folder" &&
        this.removeItemFromStructure(id, item.items)
      )
        return true;
    }
    return false;
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (song) {
          ...
          const sRef = window.storageRef(
            window.storage,
            `sharedLibs/${shareId}/${song.id}.mp3`,
          );
          ...
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              ...
              console.error(
                `Upload failed (Attempt ${attempt + 1}/3): ${song.name}`,
                err,
              );
  ...
  async loadSharedLibrary(shareId) {
    ...
    status.innerText = "Downloading Shared Library...";
    status.style.display = "inline-block";
    try {
      const snap = await window.getDb(
        window.dbRef(window.db, "sharedLibs/" + shareId),
      );
      if (snap.exists()) {
        const data = snap.val();
        if (data && data.songs) {
          for (const s of data.songs) {
            ...
            await window.player.db.add(blob, {
        ...
        status.innerText = "Library Downloaded!";
        ...
        status.innerText = "Shared Library Not Found";
        setTimeout(() => (status.style.display = "none"), 3000);
      }
    } catch (e) {
  ...
  async exportLibrary() {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `okMUSIC_Library_${Date.now()}.json`;
      a.click();
  ...
  async importLibrary(input) {
    ...
    try {
      ...
      if (data && data.songs) {
        ...
        window.ui.showToast("Invalid library file.");
      }
    } catch (e) {
      console.error(e);
      window.ui.showToast("Import Failed!");
    }
  }
}
...
class VolumeAnim {
  ...
  step() {
    ...
    if (
      Math.abs(this.targetValue - this.currentValue) < 0.1 &&
...
class okMUSICLearningController {
  ...
  processFrame(analyser) {
    ...
    if (
      totalFlux > effectiveThreshold &&
      this.framesSinceLastOnset >= this.refractoryFrames
    ) {
  ...
  learnFromTimeline(userBeatSignals) {
    ...
    for (let ub of userBeatSignals) {
      ...
      let closestFrame = this.featureHistory.reduce((prev, curr) => {
        return Math.abs(curr.trackTime - userTime) <
          Math.abs(prev.trackTime - userTime)
          ? curr
          : prev;
      ...
      if (closestFrame && Math.abs(closestFrame.trackTime - userTime) < 0.2) {
        this.applyGradientDescent(
          closestFrame.features,
          closestFrame.prediction,
          1.0,
        );
        this.alphaMultiplier = Math.max(
          1.0,
          this.alphaMultiplier - this.alphaLearningRate * 0.5,
        );
        falseNegatives++;
      }
    }
    ...
    for (let frame of this.featureHistory) {
      let predictedScore =
        frame.features[0] * this.weights[0] +
        frame.features[1] * this.weights[1] +
        frame.features[2] * this.weights[2];
      if (predictedScore > 0.05) {
        ...
        if (!isNearUserPoint) {
          ...
          this.alphaMultiplier = Math.min(
            5.0,
            this.alphaMultiplier + this.alphaLearningRate * 0.1,
          );
          falsePositives++;
        }
      }
    ...
    if (window.ui && window.ui.showToast) {
      window.ui.showToast(
        `Neural Network Updated: Fixed ${falseNegatives} misses & ${falsePositives} false positives.`,
      );
    }
  }
...
class AudioSystem {
  constructor() {
    ...
    this.bassCurve = [];
    this.initBassCurve();
    this.volAnim = new VolumeAnim("volPercentDisplay");
    setTimeout(() => this.volAnim.animateTo(1), 500);
  }
  async init() {
    if (this.ctx) return;
  ...
  impulse(d) {
    const r = this.ctx.sampleRate,
      l = r * d,
      b = this.ctx.createBuffer(2, l, r);
    for (let c = 0; c < 2; c++) {
      const d = b.getChannelData(c);
      for (let i = 0; i < l; i++)
        d[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / l, 2);
    }
    return b;
  }
  ...
  toggleGlobal(on) {
    this.globalAudio = on;
    localStorage.setItem("sv_global_audio", on);
  }
  ...
  renderCurveNodes() {
    ...
    if (!this.bassCurve || this.bassCurve.length === 0) {
      container.innerHTML =
        '<div class="text-center text-[10px] text-white/30 font-bold py-2">No points mapped. Add a node below.</div>';
      return;
    }
  ...
  setBass(v) {
    this.bassVal = v;
    window.$("bassVal").innerText = `+${v}dB`;
    this.updateDynamicBass();
    if (this.globalAudio) localStorage.setItem("sv_bass", v);
    else if (window.player && window.player.currentTrack) {
      window.player.currentTrack.bassVal = v;
      window.player.db.update(window.player.currentTrack.id, { bassVal: v });
    }
    if (window.ui && window.ui.updateMetaTags) window.ui.updateMetaTags();
    this.broadcastFx();
  }
  setReverb(v) {
    ...
    this.reverbVal = v;
    window.$("reverbVal").innerText = v + "%";
    if (this.globalAudio) localStorage.setItem("sv_reverb", v);
    else if (window.player && window.player.currentTrack) {
      window.player.currentTrack.reverbVal = v;
      window.player.db.update(window.player.currentTrack.id, { reverbVal: v });
    }
    if (window.ui && window.ui.updateMetaTags) window.ui.updateMetaTags();
    this.broadcastFx();
  }
  setVolume(v) {
    ...
    if (sl)
      sl.style.background = `linear-gradient(to right, #ff2a2a ${v * 100}%, rgba(255,255,255,0.1) ${v * 100}%)`;
    this.updateDynamicBass();
  ...
  setBaseSpeed(v) {
    ...
    window.$("pitchVal").innerText = v + "x";
    if (this.globalAudio) localStorage.setItem("sv_pitch", v);
    else if (window.player && window.player.currentTrack) {
      window.player.currentTrack.baseSpeed = parseFloat(v);
  ...
  toggleXtreme() {
    const on = window.$("xtremeToggle").checked;
    this.xtremeOn = on;
    if (this.ctx) {
      this.bass.type = on ? "peaking" : "lowshelf";
      this.bass.frequency.value = on ? 60 : 200;
      this.bass.Q.value = on ? 1.5 : 1;
    }
    this.updateDynamicBass();
    if (this.globalAudio) localStorage.setItem("sv_xtreme", on);
    else if (window.player && window.player.currentTrack) {
  ...
  broadcastFx() {
    if (
      window.partyMode &&
      window.partyMode.active &&
      window.partyMode.isHost &&
      window.partyMode.hostFxOverride
    ) {
...
class Visualizer {
  ...
  toggleMode() {
    ...
    const linesToggleLabel =
      window.$("liquidLinesToggle")?.previousElementSibling; // Span is before input
    ...
    for (let s of spans) {
      if (
        s.innerText.includes("Liquid Lines") ||
        s.innerText.includes("Smooth Following")
      ) {
        targetSpan = s;
        break;
      }
  ...
  clearFlash() {
    window.$("beat-flash").style.opacity = 0;
  }
  ...
  draw() {
    ...
    if (
      ...
      !window.audioSys.audio.paused
    ) {
      if (window.player.currentTrack.beatSignals) {
        const hit = window.player.currentTrack.beatSignals.some(
          (bt) => Math.abs(bt - t) < 0.05,
        );
    ...
    if (bgLayer) {
      ...
      if (bass > 100) {
        ...
        bgLayer.style.filter = isMobile
          ? "none"
          : `blur(30px) brightness(${1 + bgInt * 0.5})`;
      } else {
    ...
    if (
      ...
      bass > 180 &&
      bassJump > 30 &&
      performance.now() - this.lastRippleTime > 300
    ) {
  ...
  drawRing(ctx, w, h, d) {
    ...
    const lim = Math.floor(d.length * 0.6),
      step = Math.max(1, Math.floor(lim / bars));
    const sens = window.vizSens || 1.0;
    ...
    ctx.lineCap = "round";
    ctx.shadowBlur = isMobile ? 0 : 15 + this.xStrength * 30;
    const pts = [];
    ...
    if (this.smooth) {
      ...
      for (let i = 0; i < bars; i++) {
        const ang = (i / bars) * Math.PI * 2,
          ...
          x = cx + Math.cos(ang) * (r + d[idx] * sens * 1.5),
          y = cy + Math.sin(ang) * (r + d[idx] * sens * 1.5);
        path.moveTo((cx + Math.cos(ang) * r) | 0, (cy + Math.sin(ang) * r) | 0);
        path.lineTo(x | 0, y | 0);
      }
      ctx.stroke(path);
    }
  ...
  drawBars(ctx, w, h, d) {
    const cnt = 100,
      sp = w / cnt;
    const lim = Math.floor(d.length * 0.6),
      step = Math.max(1, Math.floor(lim / cnt));
    const sens = window.vizSens || 1.0;
    const isMobile =
      window.innerWidth < 800 ||
      window.innerHeight < 600 ||
  ...
  drawLiquid(ctx, w, h, ignored_d) {
    ...
    if (ghosting && !isMobile) {
      if (tMode === "rainbow") {
        ...
        renderQueue.push({
          data: this.history.getDelayedFrame(21),
          color: "#10b981",
        }); // Green
        renderQueue.push({
          data: this.history.getDelayedFrame(18),
          color: "#3b82f6",
        }); // Blue
        renderQueue.push({
          data: this.history.getDelayedFrame(15),
          color: "#a855f7",
        }); // Purple
        renderQueue.push({
          data: this.history.getDelayedFrame(12),
          color: "#f472b6",
        }); // Pink
        renderQueue.push({
          data: this.history.getDelayedFrame(9),
          color: "#ef4444",
        }); // Red
        renderQueue.push({
          data: this.history.getDelayedFrame(6),
          color: "#eab308",
        }); // Yellow
      } else {
        ...
        if (
          tMode === "gradient" &&
          window.ui.themeGrad1 &&
          window.ui.themeGrad2
        ) {
          renderQueue.push({
            data: this.history.getDelayedFrame(12),
            color: window.ui.themeGrad1,
          });
          renderQueue.push({
            data: this.history.getDelayedFrame(6),
            color: window.ui.themeGrad2,
          });
        } else if (tMode === "dominant" && window.dominantColor) {
          renderQueue.push({
            data: this.history.getDelayedFrame(12),
            color: window.dominantColor2 || "#ffffff",
          });
          renderQueue.push({
            data: this.history.getDelayedFrame(6),
            color: window.dominantColor,
          });
        } else {
          const acc =
    ...
    ctx.globalCompositeOperation = "source-over";
    const mainPts = this.getSymmetricPoints(
      cx,
      cy,
      this.history.getDelayedFrame(0),
    );
...
class CurveEditor {
  constructor() {
    ...
    this.beatSignals = [];
    this.triggers = [];
    this.presets = JSON.parse(localStorage.getItem("sv_presets")) || [
      {
        name: "Normal",
        pts: [
          { x: 0, y: 0.5 },
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if (e.key.toLowerCase() === "q") {
        ...
        this.cv.classList.add("cursor-grab");
      }
      if (e.key.toLowerCase() === "d") {
        this.isDPressed = true;
        this.cv.classList.add("cursor-delete");
      }
    ...
    window.addEventListener("keyup", (e) => {
      if (e.key.toLowerCase() === "q") {
        this.isQPressed = false;
        this.cv.classList.remove("cursor-grab");
        this.isDraggingPlayhead = false;
      }
      if (e.key.toLowerCase() === "d") {
        this.isDPressed = false;
        this.cv.classList.remove("cursor-delete");
      }
    ...
    this.cv.addEventListener("wheel", (e) => {
      ...
      if (e.shiftKey) {
        const scrollAmount =
          (e.deltaY > 0 ? 1 : -1) * (duration / this.zoom / 20);
        ...
        this.setZoom(
          Math.max(1, Math.min(20, this.zoom + (e.deltaY < 0 ? 0.5 : -0.5))),
        );
        if (this.zoom !== oldZoom) {
          const newVisibleDur = duration / this.zoom;
          this.viewOffset = Math.max(
            0,
            timeAtMouse - (mouseX / this.cv.width) * newVisibleDur,
          );
        }
      }
    });
  ...
  get isOpen() {
    return window.$("curveEditor").style.opacity === "1";
  }
  ...
  paste() {
    ...
    const anchorTime = mouseInside
      ? this.xToTime(this.mouseX)
      : window.audioSys.audio.currentTime;
  ...
  deleteSelected() {
    if (this.selectedBeats.size === 0) return;
    this.saveState();
    this.beatSignals = this.beatSignals.filter(
      (t) => !this.selectedBeats.has(t),
    );
    this.selectedBeats.clear();
    this.saveSignals();
  }
  ...
  open() {
    ...
    if (window.player.currentTrack) {
      ...
      if (window.player.currentTrack.speedPoints)
        this.pts = window.player.currentTrack.speedPoints;
      this.loadWaveform();
    }
    this.setMode("beat");
    this.startLoop();
  ...
  async loadWaveform() {
    ...
    try {
      ...
      const audioBuffer =
        await window.audioSys.ctx.decodeAudioData(arrayBuffer);
  ...
  setMode(mode) {
    ...
    if (mode === "speed") {
      ...
      window.$("editorHint").innerText =
        "Speed: Click line to add point (Q: Seek, D: Delete)";
    } else if (mode === "beat") {
      window.$("btnModeBeat").classList.add("active");
      window.$("editorHint").innerText =
        "Beat: Drag to Box Select. Ctrl+C/V to Copy/Paste. Ctrl+Z/B to Undo/Redo.";
    } else if (mode === "trigger") {
      window.$("btnModeTrigger").classList.add("active");
      window.$("editorHint").innerText =
        "Trigger: Drag green areas. Click SCAN to auto-mark beats.";
  ...
  setVerticalZoom(z) {
    this.verticalZoom = parseFloat(z);
  }
  ...
  down(e) {
    ...
    if (this.editMode === "speed") {
      ...
      if (h !== -1) {
        ...
        this.pts.push({ x: time / duration, y: 1 - p.realY / this.cv.height });
      }
      ...
      if (this.dragType === "move_beats") {
        ...
        const visibleDur = duration / this.zoom;
        const timeDelta =
          ((p.realX - this.dragStartMouse) / this.cv.width) * visibleDur;

        const staticBeats = this.beatSignals.filter(
          (t) => !this.selectedBeats.has(t),
        );
        ...
        this.beatSignals = [...staticBeats, ...currentMovedBeats].sort(
          (a, b) => a - b,
        );
      ...
      if (hit) {
        ...
        if (this.activeTrigger.startLevel === undefined)
          this.activeTrigger.startLevel = this.activeTrigger.level;
        if (this.activeTrigger.endLevel === undefined)
          this.activeTrigger.endLevel = this.activeTrigger.level;
      } else {
  ...
  findTriggerAt(mx, my) {
    ...
    for (let trig of this.triggers) {
      ...
      const sy = h / 2 - (sL * h) / 2;
      const ey = h / 2 - (eL * h) / 2;
      if (Math.abs(mx - sx) < tol && Math.abs(my - sy) < tol)
        return { trigger: trig, type: "triggerLeft" };
      if (Math.abs(mx - ex) < tol && Math.abs(my - ey) < tol)
        return { trigger: trig, type: "triggerRight" };
      if (mx >= sx && mx <= ex) {
        const t = (mx - sx) / (ex - sx);
        const yAtX = sy + (ey - sy) * t;
        if (Math.abs(my - yAtX) < tol)
          return { trigger: trig, type: "trigger" };
      }
    }
    return null;
  }
  ...
  move(e) {
    ...
    if (this.isPanning) {
      ...
      this.viewOffset = Math.max(
        0,
        Math.min(this.viewOffset + deltaT, duration - visibleDur),
      );
      this.lastMouseX = p.realX;
      return;
    }
    ...
    if (this.editMode === "speed" && this.drag !== -1) {
      ...
      else if (this.drag === this.pts.length - 1)
        this.pts[this.pts.length - 1].y = yVal;
      else
        this.pts[this.drag] = {
          x: Math.max(0, Math.min(1, time / duration)),
          y: yVal,
        };
    } else if (this.editMode === "beat") {
      if (this.dragType === "move_beats") {
  ...
  deleteAtCursor(mouseX) {
    ...
    this.beatSignals = this.beatSignals.filter(
      (t) => Math.abs(t - cursorTime) > timeTolerance,
    );
    this.selectedBeats.forEach((t) => {
      if (Math.abs(t - cursorTime) <= timeTolerance)
        this.selectedBeats.delete(t);
    });
    ...
    if (this.editMode === "trigger") {
      this.triggers = this.triggers.filter(
        (trig) => !(cursorTime >= trig.start && cursorTime <= trig.end),
      );
      this.saveSignals();
    }
  }
  ...
  seekToMouse(x) {
    ...
    if (isFinite(time))
      window.audioSys.audio.currentTime = Math.max(0, Math.min(time, dur));
  }
  ...
  scanTriggers() {
    if (!this.waveform || !this.triggers.length)
      return alert("No triggers or waveform loaded.");
    ...
    if (this.triggers.length > 0) {
      ...
      window.$("scanRefineActions").querySelector("input[type=range]").value =
        avgLvl;
    }
  }
  ...
  performScan() {
    ...
    if (this.prevBeats) {
      this.beatSignals = [...this.prevBeats];
    }
    let addedCount = 0;
    this.triggers.forEach((trig) => {
      ...
      if (startIdx >= 0 && endIdx < data.length) {
        ...
        for (let i = startIdx; i <= endIdx; i++) {
          ...
          if (amp > threshold) {
            const isDup = this.beatSignals.some(
              (b) => Math.abs(b - t) < minGap,
            );
  ...
  confirmScan() {
    window.$("scanPopup").classList.remove("active");
    this.prevBeats = null;
  }
  discardScan() {
    if (this.prevBeats) {
  ...
  updateScanGap(val) {
    this.scanMinGap = parseFloat(val);
    window.$("gapVal").innerText = this.scanMinGap + "s";
  }
  reScan() {
    this.performScan();
    window.$("scanInitialActions").classList.remove("hidden");
  ...
  draw() {
    ...
    if (this.waveform) {
      ...
      const startSample = Math.floor(
        (this.viewOffset / duration) * data.length,
      );
    ...
    if (this.editMode === "speed") {
      ...
      this.pts.forEach((p) => {
        ...
        if (x >= -10 && x <= w + 10) {
          ctx.beginPath();
          ctx.arc(x, (1 - p.y) * h, 6, 0, Math.PI * 2);
          ctx.fill();
        }
      });
    }
  ...
  getSpeedAt(p) {
    ...
    for (let i = 0; i < this.pts.length - 1; i++)
      if (p >= this.pts[i].x && p <= this.pts[i + 1].x) {
        p1 = this.pts[i];
        p2 = this.pts[i + 1];
        break;
      }
    const r = p2.x - p1.x;
    if (r === 0) return Math.max(0.25, p1.y * 2.0);
    const t = (p - p1.x) / r;
    return Math.max(0.25, (p1.y + (p2.y - p1.y) * t) * 2.0);
  }
  saveSignals() {
    if (window.player.currentTrack) {
  ...
  teachAlgorithm() {
    if (!this.waveform || !this.beatSignals.length)
      return window.ui.showToast(
        "Add some beats first to teach the algorithm!",
      );
    ...
    let optimalGap =
      gaps.length > 0 ? gaps[Math.floor(gaps.length * 0.05)] * 0.95 : 0.18;
    ...
    for (let i = lag; i < envelope.length; i++) {
      ...
      for (let j = i - lag; j < i; j++)
        sumVariance += Math.pow(filteredEnv[j] - localMean, 2);
      ...
      const isUserBeat = this.beatSignals.some(
        (bt) => Math.abs(bt - currentTime) < 0.05,
      ); // within 50ms
    ...
    let rawZScore =
      userBeatZScores.length > 0
        ? userBeatZScores[Math.floor(userBeatZScores.length * 0.1)]
        : 3.5;
    ...
    window.ui.showToast(
      `DPPD Taught! New Threshold: ${(optimalThresh * 7).toFixed(2)}`,
    );
  }
  ...
  exportAlgorithmData() {
    ...
    if (!data)
      return window.ui.showToast("You haven't taught the algorithm yet!");
    const blob = new Blob([data], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `okmusic_bass_algorithm_${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  autoGenerate() {
    if (!window.player.currentTrack || !this.waveform)
      return window.ui.showToast("Wait for track to finish loading.");
    ...
    const blob = new Blob([AutoBeatWorkerCode], {
  ...
  openSaveModal() {
    window.$("presetModal").style.opacity = "1";
    window.$("presetModal").style.pointerEvents = "auto";
    window.$("presetNameInput").value = "";
    window.$("presetNameInput").focus();
  }
  closeSaveModal() {
    window.$("presetModal").style.opacity = "0";
    window.$("presetModal").style.pointerEvents = "none";
  }
  confirmSave() {
    const n = window.$("presetNameInput").value;
    if (n) {
      this.presets.push({ name: n, pts: JSON.parse(JSON.stringify(this.pts)) });
      localStorage.setItem("sv_presets", JSON.stringify(this.presets));
      this.rendPre();
      this.closeSaveModal();
    }
  }
...
class UI {
  constructor() {
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if (e.code === "Space") {
        ...
        window.audioSys.gain.gain.value = Math.min(
          1,
          window.audioSys.gain.gain.value + 0.05,
        );
      } else if (e.code === "ArrowDown") {
        window.audioSys.gain.gain.value = Math.max(
          0,
          window.audioSys.gain.gain.value - 0.05,
        );
      } else if (e.code === "KeyM") {
  ...
  highlightActiveTrack(id) {
    ...
    // Remove old active
    el.querySelectorAll(".lib-card.active").forEach((c) =>
    ...
    if (card) {
      ...
      el.querySelectorAll(".active-parent").forEach((h) =>
        h.classList.remove("active-parent"),
      );
    }
  ...
  handleSearch(val) {
    ...
    setTimeout(() => {
      ...
      if (this.lastActiveTop !== -1 && indicator) {
        ...
        const el = window.$("libraryList");
        const target =
          el.querySelector(".lib-card.active") ||
          el.querySelector(".active-parent");
  ...
  toggleSelectMode() {
    ...
    window
      .$("selectBtn")
      .classList.toggle("text-[var(--accent)]", this.selectMode);
  ...
  updateSelectCount() {
    if (window.$("selectCountDisplay"))
      window.$("selectCountDisplay").innerText =
  ...
  async deleteSelected() {
    ...
    if (
      confirm(
        `Are you sure you want to delete ${this.selectedIds.length} items?`,
      )
    ) {
  ...
  renderLibrary(filterText = "") {
    ...
    const renderSongCard = (
        ...
        if (this.selectMode && isSelected) {
          window.libraryMgr.dragSrc = {
            items: this.selectedIds
              .map((id) => window.libraryMgr.findItem(id))
              .filter((x) => x),
            type: "multi",
          };
        } else {
      ...
      card.ondragend = () => {
        ...
        document
          ...
          .forEach((e) => e.classList.remove("drag-highlight"));
      };
      ...
      if (this.selectMode) {
        ...
        iconBox.innerHTML =
          '<i class="fa-solid fa-music text-sm opacity-50"></i>';
      }
      ...
      if (highlightQuery) {
        ...
        title.innerHTML = song.name.replace(
          regex,
    ...
    if (window.libraryMgr.structure.length === 0 && !filterText) {
      ...
      emptyState.ondragover = (e) => {
        e.preventDefault();
        emptyState.style.borderColor = "var(--accent)";
      };
      emptyState.ondragleave = () => {
        emptyState.style.borderColor = "rgba(255,255,255,0.1)";
      };
      emptyState.ondrop = (e) => {
        e.preventDefault();
  ...
  getFlatList() {
    ...
    const scan = (i) => {
      i.forEach((x) => {
        if (x.type === "song") l.push(x.id);
        else if (x.type === "folder") scan(x.items);
      });
    };
  ...
  setTrack(s) {
    ...
    if (s.art) {
      ...
      window.dominantColor = null;
      window.dominantColor2 = null;
    }
  }
  updatePlayBtn(p) {
    window.$("btnPlay").innerHTML = p
  ...
  toggleLibrary() {
    ...
    window.$("settingsPanel").classList.remove("open");
    window.$("queuePanel").classList.remove("open");
  }
  toggleSettings() {
    window.$("settingsPanel").classList.toggle("open");
    window.$("libraryPanel").classList.remove("open");
    window.$("queuePanel").classList.remove("open");
  }
  toggleQueue() {
    window.$("queuePanel").classList.toggle("open");
    window.$("settingsPanel").classList.remove("open");
    window.$("libraryPanel").classList.remove("open");
    this.renderQueue();
  }
  ...
  renderQueue() {
    ...
    if (window.player.queue.length === 0) {
      list.innerHTML =
        '<div class="text-[10px] text-white/30 text-center uppercase tracking-widest mt-4">Queue is empty</div>';
      return;
    }
    window.player.queue.forEach((id, idx) => {
      ...
      const card = document.createElement("div");
      card.className =
        "w-full bg-white/5 border border-white/10 p-3 rounded-lg flex items-center gap-3 relative";
      card.innerHTML = `
  ...
  setThemeMode(mode) {
    ...
    if (solidContainer)
      solidContainer.style.display =
        mode === "solid" || mode === "solid-fill" ? "flex" : "none";
    if (gradContainer)
      gradContainer.style.display = mode === "gradient" ? "flex" : "none";
    ...
    if (txtLabel) {
      if (mode === "solid") txtLabel.innerText = "Solid Accent Color";
        ...
        txtLabel.innerText = "Custom 2-Color Gradient";
  ...
  openEdit(t, id) {
    this.editType = t;
    this.editId = id;
    window.$("trackEditor").style.opacity = "1";
    window.$("trackEditor").style.pointerEvents = "auto";
    ...
    if (t === "song") {
      ...
      if (window.$("editArtUrl"))
        window.$("editArtUrl").value = s ? s.customArtUrl || "" : "";
  ...
  closeEditor() {
    window.$("trackEditor").style.opacity = "0";
    window.$("trackEditor").style.pointerEvents = "none";
    this.tempArt = null;
  }
  previewEditArt(i) {
    const f = i.files[0];
    const r = new FileReader();
    r.onload = (e) => (this.tempArt = e.target.result);
    r.readAsDataURL(f);
  }
  async saveEditor() {
    ...
    if (this.editType === "song") {
      const u = { name: n };
      const customUrl = window.$("editArtUrl")
        ? window.$("editArtUrl").value.trim()
        : "";
      if (this.tempArt) u.art = this.tempArt;
      else if (customUrl) {
        u.customArtUrl = customUrl;
        u.art = customUrl;
      }
  ...
  setBgBrightness(v) {
    window.$("bg-layer").style.opacity = v;
    localStorage.setItem("sv_bg_bright", v);
  }
  updateMetaTags() {
    const c = window.$("metaTags");
    c.innerHTML = "";
    if (window.audioSys.baseSpeed != 1)
      c.innerHTML += `<span class="tag" style="background: rgba(168, 85, 247, 0.15); border-color: rgba(168, 85, 247, 0.3); color: #e879f9;">WARP ${window.audioSys.baseSpeed}x</span>`;
    if (window.audioSys.bassVal > 0)
      c.innerHTML += `<span class="tag" style="background: rgba(239, 68, 68, 0.15); border-color: rgba(239, 68, 68, 0.3); color: #f87171;">BASS +${window.audioSys.bassVal}</span>`;
    if (window.audioSys.reverbVal > 0)
      c.innerHTML += `<span class="tag" style="background: rgba(56, 189, 248, 0.15); border-color: rgba(56, 189, 248, 0.3); color: #7dd3fc;">REVERB ${window.audioSys.reverbVal}%</span>`;
    if (window.player && window.player.autoDj)
      c.innerHTML += `<span class="tag" style="background: rgba(34, 197, 94, 0.15); border-color: rgba(34, 197, 94, 0.3); color: #4ade80;">AUTO DJ</span>`;
  }
  ...
  showSyncPopup(title, pct) {
    ...
    if (title.includes("Uploading") || title.includes("Streaming")) {
      icon.className =
        "fa-solid fa-cloud-arrow-up text-3xl text-[var(--accent)] mb-2 animate-bounce";
    } else {
      icon.className =
        "fa-solid fa-cloud-arrow-down text-3xl text-[var(--accent)] mb-2 animate-bounce";
    }
  ...
  updateActiveIndicator() {
    ...
    if (
      Math.abs(actualTop - this.lastActiveTop) < 1 &&
      Math.abs(newHeight - indicator.offsetHeight) < 1
    ) {
      indicator.style.display = "block";
      indicator.style.top = actualTop + "px";
      indicator.style.height = newHeight + "px";
...
class Player {
  ...
  play(id, contextList = null, renderList = true) {
    ...
    if (window.audioSys.globalAudio) {
      ...
      const bass =
        s.bassVal !== undefined
          ? s.bassVal
          : localStorage.getItem("sv_bass") || 0;
      const spd =
        s.baseSpeed !== undefined
          ? s.baseSpeed
          : localStorage.getItem("sv_pitch") || 1.0;
      const rev =
        s.reverbVal !== undefined
          ? s.reverbVal
          : localStorage.getItem("sv_reverb") || 0;
      const xtr =
        s.xtremeOn !== undefined
          ? s.xtremeOn
          : localStorage.getItem("sv_xtreme") === "true";
  ...
  togglePlay() {
    ...
    if (!this.currId) {
      ...
      if (flat.length) this.play(flat[0], flat);
      ...
      if (window.partyMode && window.partyMode.active)
        window.partyMode.broadcast({
          type: "STATUS",
          playing: true,
          time: window.audioSys.audio.currentTime,
        });
    } else {
      window.audioSys.audio.pause();
      window.ui.updatePlayBtn(false);
      if (window.partyMode && window.partyMode.active)
        window.partyMode.broadcast({
          type: "STATUS",
          playing: false,
  ...
  onTick() {
    ...
    if (window.audioSys.audio.duration) {
      const pct =
        window.audioSys.audio.currentTime / window.audioSys.audio.duration;
      window.audioSys.audio.playbackRate =
        window.curveEditor.getSpeedAt(pct) * window.audioSys.baseSpeed;

      if (
        this.autoDj &&
        window.audioSys.audio.duration - window.audioSys.audio.currentTime <
          5 &&
        !this.fading
      ) {
        this.fading = true;
        ...
        if (!window.fadeAudio) {
          ...
          const src = window.audioSys.ctx.createMediaElementSource(
            window.fadeAudio,
          );
        ...
        const fadeOut = setInterval(() => {
          if (oldVol > 0.02) {
            ...
            window.fadeGain.gain.value = 0;
            clearInterval(fadeOut);
            window.fadeAudio.pause();
          }
        }, 50);
        ...
        const fadeIn = setInterval(() => {
          if (newVol < 1.0) {
            newVol += 0.02;
            window.audioSys.gain.gain.value = newVol;
          } else {
            window.audioSys.gain.gain.value = 1.0;
  ...
  onEnd() {
    if (this.loop) this.play(this.currId);
    else this.next();
  }
  toggleRepeat() {
    this.loop = !this.loop;
  ...
  loadSettings() {
    ...
    if (b) {
      ...
      window.audioSys.setBass(b);
    }
    const r = localStorage.getItem("sv_reverb");
    if (r) {
      document.querySelectorAll(".setting-slider")[2].value = r;
      window.audioSys.setReverb(r);
    }
    const p = localStorage.getItem("sv_pitch");
    if (p) {
      document.querySelectorAll(".setting-slider")[3].value = p;
      window.audioSys.setBaseSpeed(p);
    }
    const c = localStorage.getItem("sv_theme");
    if (c) {
      window.ui.setTheme(c);
      if (window.$("colorPicker")) window.$("colorPicker").value = c;
    }
    const tm = localStorage.getItem("sv_theme_mode");
    if (tm) {
      window.ui.setThemeMode(tm);
      if (window.$("themeModeSelector"))
        window.$("themeModeSelector").value = tm;
    } else {
      window.ui.setThemeMode("solid");
    }
    const g1 = localStorage.getItem("sv_grad_1");
    const g2 = localStorage.getItem("sv_grad_2");
    ...
    const xtr = localStorage.getItem("sv_xtreme") === "true";
    window.$("xtremeToggle").checked = xtr;
    window.audioSys.toggleXtreme();
    const glob = localStorage.getItem("sv_global_audio") === "true";
    window.$("globalAudioToggle").checked = glob;
    window.audioSys.globalAudio = glob;
    const sync = localStorage.getItem("sv_party_sync");
    if (sync) {
      if (window.$("syncOffsetSlider")) {
        window.$("syncOffsetSlider").value = sync;
        window.$("syncOffsetVal").innerText = sync + "ms";
      }
    }
  }
}
...
class PartyMode {
  ...
  connect() {
    ...
    try {
      ...
      window.ui.showToast("Connection Error");
  ...
  updateCardProgress(id, pct) {
    ...
    if (card) {
      ...
      if (!prog) {
        ...
        prog.style =
          "position:absolute; left:0; top:0; width:0%; height:100%; background:var(--accent); opacity:0.15; pointer-events:none; transition:width 0.1s linear, opacity 0.3s; z-index:0; border-radius:inherit;";
    ...
    dc.onmessage = async (e) => {
      if (typeof e.data === "string") {
        ...
        if (msg.type === "META") {
          ...
          await window.player.db.add(blob, {
            beatSignals: incomingMeta.beatSignals,
            speedPoints: incomingMeta.speedPoints,
          });
          await window.player.loadLib();
        ...
        if (incomingMeta) {
          const pct = Math.round(
            (incomingChunks.length / incomingMeta.totalChunks) * 100,
          );
  ...
  handleMessage(data) {
    ...
    if (data.type === "ANSWER" && this.isHost) {
      ...
      if (pc)
        pc.setRemoteDescription(
          new RTCSessionDescription(JSON.parse(data.sdp)),
        );
    }
    if (data.type === "ICE") {
      const pc = this.peers[data.sender];
    ...
    if (data.type === "MANIFEST" && !this.isHost) {
      const missing = data.ids.filter(
        (id) => !window.player.songs.some((s) => s.id === id),
      );
    ...
    if (data.type === "WANT_LIST" && this.isHost) {
      ...
      if (dc && dc.readyState === "open") {
        ...
        window.ui.showToast(
          "Syncing " + data.missing.length + " missing tracks to Client!",
        );
        const pushSeq = async () => {
    ...
    if (data.type === "PLAY") {
      ...
      let song = window.player.songs.find(
        (s) =>
          s.id === data.id ||
          s.name === data.name ||
          (s.name &&
            s.name.replace(".mp3", "") === data.name.replace(".mp3", "")),
      );
      if (song) {
      ...
      if (data.playing && window.audioSys.audio.paused)
        window.player.togglePlay();
      else if (!data.playing && !window.audioSys.audio.paused)
        window.player.togglePlay();
      ...
      if (data.time && data.timestamp) {
        ...
        if (Math.abs(drift) > 0.3) {
          ...
          window.audioSys.audio.playbackRate =
            (window.audioSys.baseSpeed || 1.0) * slewRate;
        } else {
          window.audioSys.audio.playbackRate = window.audioSys.baseSpeed || 1.0;
        }
      }
      ...
      if (data.bass !== undefined) {
        sl[1].value = data.bass;
        window.audioSys.setBass(data.bass);
      }
      if (data.reverb !== undefined) {
        sl[2].value = data.reverb;
        window.audioSys.setReverb(data.reverb);
      }
      if (data.speed !== undefined) {
        sl[3].value = data.speed;
        window.audioSys.setBaseSpeed(data.speed);
      }
      if (data.xtreme !== undefined) {
        window.$("xtremeToggle").checked = data.xtreme;
        window.audioSys.toggleXtreme();
      }
...
class VoiceCommander {
  constructor() {
    ...
    if ("webkitSpeechRecognition" in window) {
      ...
      this.rec.onend = () => {
        if (this.listening) this.rec.start();
      };
    }
  }
  toggle() {
  ...
  if (cached) {
    try {
      ...
      if (parsed.threshold && sensSlider) {
        ...
        if (document.getElementById("dppdSensVal"))
          document.getElementById("dppdSensVal").innerText = sensDisplay;
      }
      if (parsed.minGap && gapSlider) {
        const gapDisplay = parsed.minGap;
        gapSlider.value = gapDisplay;
        if (document.getElementById("dppdGapVal"))
          document.getElementById("dppdGapVal").innerText = gapDisplay + "s";
      }
    } catch (e) {}
  }
};
