        ...
        class LibraryManager {
            ...
            async shareLibrary() {
                ...
                try {
                    ...
                    for(const id of flatList) {
                        ...
                        if(song) {
                            ...
                            songsToShare.push({
                                id: song.id, name: song.name, data: base64,
                ...
                } catch(e) { alert("Share Failed: " + e.message); console.error(e); }
        ...
        class AudioSystem {
            constructor() { this.ctx=null; this.audio=new Audio(); this.audio.crossOrigin="anonymous"; this.audio.addEventListener('timeupdate',()=>window.player.onTick()); this.audio.addEventListener('ended',()=>window.player.onEnd()); this.baseSpeed=1.0; this.bassVal=0; this.reverbVal=0; this.globalAudio=false; this.xtremeOn=false; }
            ...
            setBass(v){
                ...
                else if(window.player.currentTrack) { window.player.currentTrack.bassVal=v; window.player.db.update(window.player.currentTrack.id, {bassVal:v}); }
            ...
            setReverb(v){
                ...
                else if(window.player.currentTrack) { window.player.currentTrack.reverbVal=v; window.player.db.update(window.player.currentTrack.id, {reverbVal:v}); }
                window.ui.updateMetaTags();
            }
            ...
            setBaseSpeed(v){
                ...
                else if(window.player.currentTrack) { window.player.currentTrack.baseSpeed=parseFloat(v); window.player.db.update(window.player.currentTrack.id, {baseSpeed:parseFloat(v)}); }
                window.ui.updateMetaTags();
            }
            toggleXtreme(){
                ...
                else if(window.player.currentTrack) { window.player.currentTrack.xtremeOn=on; window.player.db.update(window.player.currentTrack.id, {xtremeOn:on}); }
            }
        };
            ...
            draw(){
                ...
                if (bgLayer) {
                     ...
                     if (bass > 100) {
                          ...
                          bgLayer.style.filter = `blur(30px) brightness(${1 + bgInt * 0.5})`;
                     } else {
                          bgLayer.style.opacity = baseOp;
                          bgLayer.style.filter = `blur(30px) brightness(1)`;
                     }
                }
            ...
            renderLibrary(filterText = "") {
                ...
                const renderSongCard = (item, song, parentList, idx, container, isRoot = false, highlightQuery = null, emptyImg) => {
                    ...
                    card.ondragstart = e => {
                        ...
                        proxy.innerHTML = `<i class="fa-solid fa-music"></i><span>${song.name}</span>`;
                        document.body.appendChild(proxy);
                        
                        // 2. Set it as the drag image (Offset centers cursor roughly)
                        e.dataTransfer.setDragImage(proxy, 20, 22);
                        
                        // 3. Remove from DOM immediately (browser takes snapshot)
                    ...
                    card.ondrop = e => {
                        ...
                        // 2. Adjust target index if in same list and removed before target
                        let insertIdx = tgt.idx;
                        if (src.parentList === tgt.list && src.idx < tgt.idx) {
                             insertIdx--;
                        }
        ...
        class Player {
            ...
            play(id, contextList=null, renderList = true){
                ...
                // Party Mode Broadcast Loop (Base64 Mode Chunked)
                if(window.partyMode && window.partyMode.active && window.db) {
                    const reader = new FileReader();
                    reader.onload = async (e) => {
                        const base64 = e.target.result;
                        const chunkSize = 150000; // 150KB chunks
                        const chunks = Math.ceil(base64.length / chunkSize);
                        
                        await window.setDb(window.dbRef(window.db, `partyData/${window.partyMode.sessionId}`), null);
                        
                        for(let i=0; i<chunks; i++) {
                            const chunk = base64.substr(i * chunkSize, chunkSize);
                            await window.setDb(window.dbRef(window.db, `partyData/${window.partyMode.sessionId}/${i}`), chunk);
                        }
                        
                        window.partyMode.broadcast({
                            type: 'PLAY', id: s.id, name: s.name, chunks: chunks,
        ...
        class PartyMode {
            ...
            connect() {
                ...
                } catch(e) { console.error("Party Mode error", e); }
            ...
            broadcast(data) {
                ...
                window.setDb(window.dbRef(window.db, 'sessions/' + this.sessionId), {
                    ...data, timestamp: Date.now(), sender: window.player.myId
                });
            }
            handleMessage(data) {
                if(data.type === 'PLAY') {
                    let song = window.player.songs.find(s => s.id === data.id || s.name === data.name);
                    if (song) {
                        ...
                        const downloadNext = async (idx) => {
                            status.innerText = `Buffering: ${Math.round((downloaded / data.chunks) * 100)}%`;
                            if(idx >= data.chunks) {
                                status.innerText = "Processing...";
                                fetch(fullString).then(r=>r.blob()).then(blob=>{
                                ...
                                }).catch(e=>{ console.error("Download failed", e); this.isDownloading = false; });
                                return;
                            }
                            ...
                            if(snap.exists()) {
                                ...
                                setTimeout(() => downloadNext(idx), 500); 
                            }
                        };
                        downloadNext(0);
                    }
                }
                else if(data.type === 'STATUS' && !this.isDownloading) {
